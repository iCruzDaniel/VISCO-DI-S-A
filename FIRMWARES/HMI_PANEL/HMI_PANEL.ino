...
void setup() {
  ...
  if (!SD.begin(chipSelect)) {
    ...
    while (true); // Detiene el programa aquí para que veas el error
  }
  ...
  switch (currentState) {
    ...
    case CONFIG_VARS_MENU:
      if (key == 'U' || key == 'D') selectedOption = (selectedOption + 1) % 2, showConfigVarsMenu();
        ...
        if (selectedOption == 0) {
          ...
          inputBufferLen = 0; inputBuffer[0] = '\0'; // Limpiar buffer
          showEditTemp();
        }
    ...
    case EDIT_TEMP:
      if ((key >= '0' && key <= '9') && inputBufferLen < (MAX_INPUT_BUFFER_LEN - 1)) {
        ...
        if (newTemp >= 0 && newTemp <= 100) {
          ...
          delay(1500);
          inputBufferLen = 0; inputBuffer[0] = '\0'; // Resetear buffer
          showEditTemp();
        }
      }
      break;
    ...
    case SPHERE_POSITION_QUESTION:
      if (key == 'N' && selectedOption == 0) {
        ...
        Serial.print(F("SphereInPosition: ")); Serial.println(sphereInPosition); 
